...
| # | 内容 | 说明 |
|---|------|------|
| 6.3.1 | 打字命中反馈 | 字母命中时弹出放大+消散、植物微跳、连击进度闪光 |
| 6.3.2 | 打字错误反馈 | 屏幕微震、当前字母红色闪烁 |
| 6.3.3 | 连击结算演出 | 弹道发射蓄力闪光、命中爆炸扩散圈；满链大招屏幕震动+特殊粒子 |
| 6.3.4 | 僵尸受击/死亡 | 受击闪白+轻微后退；死亡击飞+淡出/碎裂 |
| 6.3.5 | 屏幕震动系统 | 通用 Screen Shake，大伤害/大招/Boss 时触发 |
...
| # | 内容 | 说明 |
|---|------|------|
| 6.3.6 | 草坪网格+泥土路径 | 经典 PvZ 标志性草坪格子，给战场空间感和层次感 |
| 6.3.7 | 植物简笔画风格 | Canvas 画圆+弧线绘制卡通植物（眼睛、嘴巴、叶子），替代纯色矩形 |
| 6.3.8 | 僵尸简笔画风格 | 路障僵尸带路障、旗手带旗、各类型有形态差异，替代纯色矩形 |
| 6.3.9 | 弹道特效差异化 | 冰弹冰晶拖尾、火弹火焰粒子、电弹闪电弧线、击退弹冲击波 |
| 6.3.10 | 僵尸行走动画 | 简单左右摇晃/手臂摆动，让色块"活起来" |
...
| # | 内容 | 说明 |
|---|------|------|
| 6.3.11 | 场景过渡动画 | 淡入淡出或推入推出，消除场景硬切 |
| 6.3.12 | 结算数据动态揭晓 | 数字从 0 跳到最终值、星星逐颗掉落、称号弹出放大 |
| 6.3.13 | 波次开始/结束演出 | "Wave N Start!" 文字飞入+缩放，给每波节奏感 |
| 6.3.14 | 菜单背景动态 | 缓慢飘动的僵尸剪影或植物摇摆，让菜单不死板 |

#### P3 — 锦上添花

| # | 内容 | 说明 |
|---|------|------|
| 6.3.15 | 简易粒子系统 | 可复用于击杀碎片、连击火花、弹道拖尾、元素特效 |
| 6.3.16 | 伤害飘字 | 击杀时伤害数字飘起消散 |
| 6.3.17 | 植物待机微动 | 轻微上下浮动或摇摆，增加生命感 |
| 6.3.18 | 实体阴影 | 实体下方半透明椭圆阴影，增加立体感 |

### 完成标准

- [ ] 成就能触发和展示
- [ ] 植物图鉴能查看已解锁植物
- [ ] 打字命中/错误有即时视觉反馈，手感令人满足
- [ ] 僵尸受击闪白、死亡有击飞/淡出动效
- [ ] 连击结算和满链大招有爆发演出
- [ ] 植物和僵尸有辨识度（非纯色矩形），符合"卡通蠢萌"定位
- [ ] 不同元素弹道视觉差异明显
- [ ] 场景切换有过渡动画，结算有动态揭晓
- [ ] 打击反馈有手感（音效 + 动画配合）
- [ ] 难度曲线平滑，不出现断崖
